...
email_instance_template = """Instance {{ instance.name }} {{ scheme }}://{{ host }}{% url 'instance' pk=instance.id %} {% if instance.overall_status != instance.PASSING_STATUS %}alerting with status: {{ instance.overall_status }}{% else %}is back to normal{% endif %}.
...
class EmailAlert(AlertPlugin):
    ...
    def send_instance_alert(self, instance, users, duty_officers):
        ...
        if instance.overall_status != instance.PASSING_STATUS:
            ...
            subject = 'Instance back to normal: %s' % (service.name,)
